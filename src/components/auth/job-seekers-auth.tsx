...
export default function AuthPage() {
  ...
  const handleRegister = async () => {
    try {
      ...
      const response = await axiosInstance.post("/seeker_login.php", {
        email: registerData.email,
        password: registerData.password,
      });
      const loginData = await response.data;
      localStorage.setItem("seeker-token", loginData.token);
  ...
  const handleLogin = async () => {
    try {
      setIsSubmitting(true);
      const res = await axiosInstance.post("/seeker_login.php", loginData);
      const data = await res.data;
      toast.success(data.message);
      localStorage.setItem("seeker-token", data.token);
      router.push(lang === "ja" ? "/job-seekers" : "/en/job-seekers");
    } catch (error: any) {
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-2xl bg-white/10 p-4">
              <p className="text-2xl font-semibold text-white">
                {content.searchCard.title}
              </p>
              <p className="mt-2 text-sm text-slate-300">
                {content.searchCard.description}
              </p>
            </div>
            <div className="rounded-2xl bg-white/10 p-4">
              <p className="text-2xl font-semibold text-white">
                {content.applyCard.title}
              </p>
              <p className="mt-2 text-sm text-slate-300">
                {content.applyCard.description}
              </p>
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto w-full max-w-md">
            <div className="inline-flex rounded-2xl bg-white/5 p-1 border border-white/10 mb-8">
              <button
                ...
                className={`relative px-5 py-2.5 rounded-xl cursor-pointer text-sm font-medium transition ${
                  mode === "login"
                    ? "text-white"
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 sm:p-8 shadow-xl">
      ...
      >
        ...
        <button
          type="submit"
          className="w-full rounded-2xl bg-sky-500 px-4 py-3 cursor-pointer text-sm font-semibold text-white shadow-lg shadow-sky-500/20 transition hover:translate-y-[-1px]"
        >
          {isSubmitting
            ? lang === "ja"
              ? "読み込み中..."

...
export default function AuthPage() {
  ...
  const handleLogin = async () => {
    try {
      ...
      const res = await axiosInstance.post("/admin_login.php", loginData);
            ...
            <label key={field.label} className="block">
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 focus-within:border-sky-400 transition">
                ...
                >
                  {field.name === "password" &&
                    (showPassword ? (
                      <EyeOff className="h-4 w-4 text-slate-300 hover:text-slate-500 transition" />
                    ) : (
                      <Eye className="h-4 w-4 text-slate-300 hover:text-slate-500 transition" />
                    ))}
                </button>
              </div>
            </label>
